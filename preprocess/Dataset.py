...
class EventData(torch.utils.data.Dataset):
    ...
    def __init__(self, data, opt, split):
        ...
        """
        self.time = [[elem['time_since_start'] for elem in inst]
                     for inst in data]
        self.time_gap = [[elem['time_since_last_event'] for elem in inst[1:]]
                         for inst in data]
        # plus 1 since there could be event type 0, but we use 0 as padding
        self.event_type = [[elem['type_event'] + 1 for elem in inst]
                           for inst in data]
        ...
        if split == 'train':
            ...
            if opt.normalize == 'normal':
                ...
                self.time_gap = [[elem / mean_data for elem in inst]
                                 for inst in self.time_gap]
                self.time = [[elem / mean_data for elem in inst]
                             for inst in self.time]
            if opt.normalize == 'log':
                ...
                time_flat = (np.log(time_flat + 1e-9) -
                             mean_log_data) / var_log_data
                print(time_flat.mean())
                opt.mean_data = mean_data
                ...
                self.time_gap = [[
                    (math.log(elem / opt.mean_data + 1e-9) - opt.mean_log_data)
                    / opt.var_log_data for elem in inst
                ] for inst in self.time_gap]
            ...
            opt.time_quantile = np.array(
                [np.quantile(time_flat, i * 0.1) for i in range(1, 10)])
        else:
            # normalize
            if opt.normalize == 'normal':
                self.time_gap = [[elem / opt.mean_data for elem in inst]
                                 for inst in self.time_gap]
                self.time = [[elem / opt.mean_data for elem in inst]
                             for inst in self.time]
            if opt.normalize == 'log':
                self.time_gap = [[
                    (math.log(elem / opt.mean_data + 1e-9) - opt.mean_log_data)
