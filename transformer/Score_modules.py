...
def get_obj(t_var, score, non_pad_mask):
    grad_t = torch.autograd.grad(score.sum(), t_var,
...
class intensity_encode(nn.Module):
    ...
    def __init__(self, d_model, d_inner, num_types, encoder, config):
        ...
        self.base_layer = nn.Sequential(nn.Linear(d_model, d_inner, bias=True))

        self.affect_layer = nn.Sequential(
            nn.Linear(d_model, d_inner, bias=True), nn.Tanh())
        self.intensity_layer = nn.Sequential(nn.Linear(d_inner, 1, bias=True),
...
class score_encode(nn.Module):
    ...
    def __init__(self, d_model, d_inner, num_types, config):
        ...
        self.affect_layer = nn.Sequential(
            nn.Linear(d_model, d_inner, bias=True), nn.Tanh())
        self.score_layer = nn.Sequential(nn.Linear(d_inner, 1, bias=True),
                                         nn.Softplus(beta=1.0))
    ...
    def get_score(self, t, event_type, non_pad_mask, idx=None):
        ...
        score = torch.tanh(
            self.affect[:, :-1, None, :] * t.unsqueeze(3) +
            self.base[:, :-1, None, :])  # batch*len-1*1/num_samples*d_model
        score = self.score_layer(score).squeeze(
            3)  # (batch*len-1*1/num_samples)
...
class Predictor_with_time(nn.Module):
    ...
    def __init__(self, dim, num_types):
        ...
        self.affect_layer = nn.Sequential(
            nn.Linear(dim, int(dim / 2), bias=True), nn.Tanh())
